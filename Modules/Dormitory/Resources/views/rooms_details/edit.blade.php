...
@php
    ...
    $formattedDate = Carbon::parse($details->date_buy)->format('d/m/Y');
@endphp
@section('content')
    ...
    <div class="col-sm-8 col-md-5">
        <div class="card card-table" style="border-radius: 15px;">
            ...
            <div class="card-body" style="padding: 20px;">
                <form method="POST" action="{{route('dormitorys.rooms_details.update',$details->id)}}" enctype="multipart/form-data">
                    @csrf
                    <div class="form-group">
                        <label for="name">ชื่อ</label>
                        <input type="text" name="name" id="name" value="{{$details->name}}" required>
                    </div>
                    <div class="form-group">
                        <label for="price">ราคา</label>
                        <input type="number" name="price" id="price" value="{{$details->price}}" maxlength="50" pattern="[A-Za-z0-9ก-๙\s]+" title="กรุณาใส่เฉพาะตัวอักษรและตัวเลข" required>
                    </div>
                    <div class="form-group">
                        <label for="date_buy">วันที่ซื้อ</label>
                        <input type="text" name="date_buy" id="date_buy" class="form-control datepicker" value="{{ Carbon::parse($details->date_buy)->format('d-m-Y') }}" required>
                        {{-- <input type="text" name="date_buy" id="date_buy" value="{{ $formattedDate }}" required> --}}
                    </div>
                    <div class="form-group">
                        <label for="details">รายละเอียด</label>
                        <input type="text" name="details" id="details" value="{{$details->details}}" class="large-input" required>
                    </div>
                    <div class="form-group">
                        <label for="img_details">รูปภาพประกอบ</label>
                        @if($details->img_details)
                            <img src="{{ asset('storage/' . $details->img_details) }}" id="img_details-preview" style="width: 50px; height: 50px;  display: block; margin-bottom: 10px;">
                        @else
                    ...
                    <div class="form-group text-right">
                        ...
                        <a href="{{ route('dormitorys.rooms_details.delete', $details->id) }}" class="btn btn-danger btn-rounded" style="float:right; margin-right: 20px;">
